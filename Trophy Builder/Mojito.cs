...
namespace Trophy_Builder
{
    public class Mojito:Button
    {
        ...
        {
            spriteBatch.DrawString(font, "Mojito: ", new Vector2(x -75, y + 10), Color.White, 0, new Vector2(0, 0), 1f, SpriteEffects.None, bttnTestLevel);

            if (listOfTrophyItems.Count > trophySelected)
            {
                if (listOfTrophyItems[trophySelected].trophyLevel == 2)
                {
        ...
        public void ActivateTrophyLevel()
        {
            if (listOfTrophyItems.Count > trophySelected)
            {
                listOfTrophyItems[trophySelected].trophyLevel = 2;
